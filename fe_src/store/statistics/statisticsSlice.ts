...
>(
  ...
  async (requestData, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/statistics/delivery.php`, requestData);
...
>(
  ...
  async (requestData, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/statistics/lastlogin.php`, requestData);
      return response.data;
    } catch (e) {
      return rejectWithValue(e.response.data);
    }
