...
export const deleteDocument = async (id: number) => {
  const response = await axios.post(`${import.meta.env.VITE_API_URL}/document/delete.php`, { documentId: id });
...
export const fetchDocuments = createAsyncThunk<DocumentT[], FetchDocumentsRequestT>(
  ...
  async (requestData, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/document/list.php`, requestData);
...
export const isDocumentAvailable = async (documentId: number) => {
  const response = await axios.head(`${import.meta.env.VITE_API_URL}/document/view.php?documentId=${documentId}`);
